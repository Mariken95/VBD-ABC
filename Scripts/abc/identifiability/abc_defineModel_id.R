...
calculate.ss_sim <- function(params, verbose=0) {
  ...
  ### 1. specify which dispersal scenario and therefore which ldata and events version to use
  ...
  FOI.n <- rep(params[['FOI.n']], n/3)
  FOI.m <- rep(params[['FOI.m']], n/3)
  FOI.s <- rep(params[['FOI.s']], n/3)
  ...
  ### 3. run model
  model <- buildModel(transitions = transitions, compartments = compartments, ldata = ldata, E = E, N = N, v0 = v0, tspan = 92:2557, 
