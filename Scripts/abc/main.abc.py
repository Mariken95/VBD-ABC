...
def write_submission_script(filename, args, scenario, datasetn):
    ...
    submit_str = "#!/bin/bash\n\n"
    submit_str += "#SBATCH --comment=abc \n"
    submit_str += "#SBATCH --partition=fat_rome \n"
    submit_str += "#SBATCH --time=00-26:00:00 \n"
    submit_str += "#SBATCH --job-name=abc_%d.%d \n" % (scenario, datasetn)
    submit_str += "#SBATCH --output=abc-%d.%d.txt \n" % (scenario, datasetn)
    submit_str += "#SBATCH --error=error_abc-%d.%d.txt \n" % (scenario, datasetn)
...
parameterSet = [
    # CHAIN 1
# set 1 (A chain 1) 
' userParameterFile=paste("abc_userParameters_A_1.R"); previous_gens=NA; previous_epsilons=NA;',
