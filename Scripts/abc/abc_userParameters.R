...
# MODEL_LIST <- list() # Must be defined by the user in the MODELS_DEFINITION_FILEPATH
# runID <- "runs_20240702_"
# indexnr <- "1"
# scenarioName <- "A"
...
if (scenarioName=="A") {
  ...
  MODEL_DEFINITION_FILEPATH <- "abc_defineModel_A.R"
...
} else if (scenarioName=="C.sc20") {
    
    MODEL_DEFINITION_FILEPATH <- "abc_defineModel_C.sc20.R"
    
    PRIOR_DIST <- list("baseline" = list(c("vertical_transm", "unif", 0, 0.3), c("transmissionProbHM", "unif", 0, 1),
                                         c("scalingParameter", "unif", 0.03, 2), c("disIndMortality", "unif", 0.1, 1),
                                         c("FOI", "unif", 0, 0.3), c("lifespanR", "unif", 365, 7305),
                                         c("bitingPref", "unif", 0, 25)))
    
} else if (scenarioName=="S2") {
  
  MODEL_DEFINITION_FILEPATH <- "abc_defineModel_A.R"
  
  PRIOR_DIST <- NA
  
} else if (scenarioName=="S3") {
  
  MODEL_DEFINITION_FILEPATH <- "abc_defineModel_S3.R"
  
  PRIOR_DIST <- list("baseline" = list(c("vertical_transm", "unif", 0, 0.3), c("transmissionProbHM", "unif", 0, 1),
                                       c("transmissionProbMR", "unif", 0, 1),
                                       c("scalingParameter", "unif", 0.03, 2), c("disIndMortality", "unif", 0.1, 1),
                                       c("FOI", "unif", 0, 0.3), c("lifespanR", "unif", 365, 7305),
                                       c("bitingPref", "unif", 0, 25)))    
  ...
  MODEL_DEFINITION_FILEPATH <- NA
  
  PRIOR_DIST <- NA
  
}

MAX_NUMBER_OF_GEN <- 12
...
ss_obs <- list(deadbird_year_vector, PCRprev_likelihood$prevalence, sero_likelihood$prevalence, 
        dead.prev_likelihood$prevalence, popSize$pop)
